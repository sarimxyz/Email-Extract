...
class FCExtractedEmail(Document):
	def autoname(self):
		...
		timestamp = (
			received.strftime("%d-%m-%Y_%I-%M %p")
			if received
			else frappe.utils.now_datetime().strftime("%d-%m-%Y_%I-%M %p")
		)
...
def process_received_emails_to_trip_requests():
	...
	"""
	# 1️⃣ Initialize Claude client  (lazy importing so DocType sync works without anthropic installed)
    ...
    """
		try:
			...
			frappe.log_error(
				f"AI Validation Error for {comm['name']}: {e!s}", "Cab Booking - Validation Error"
			)
			continue
		...
		try:
			...
			frappe.log_error(
				f"Failed to create extracted email {comm['name']}: {ex!s}", "Cab Booking - Insert Error"
			)
			print(f"❌ Failed to create extracted email {comm['name']}: {ex!s}")
			continue
		...
		try:
			...
			email_doc.trip_request_error = f"Claude API call failed: {e!s}"
			email_doc.save()
			frappe.db.commit()
			continue
		...
		try:
			...
			email_doc.trip_request_status = "Failed"
			email_doc.trip_request_error = f"JSON decode error: {e!s}"
