def get_default_fc_cab_settings_prompt() -> str:
	"""
	Default `FC Cab Settings.prompt` template.

	The app replaces the `{email_text}` placeholder with the cleaned email body
	before calling the extractor LLM.
	"""

	return '''You are a data extraction assistant. Given the plain text of an email, output ONLY a single valid JSON object (no preface, no comments, no markdown, no code fences).
